...
int main( int argc, char* argv[] ) {
    ...
    if( argc < 4 ){
        std::cerr << "usage: " << argv[0] << " [size] [seed] [print 1/0]" << std::endl;
        exit(-1 );
    }

    { std::stringstream ss1( argv[1] ); ss1 >> size; }
    { std::stringstream ss1( argv[2] ); ss1 >> seed; }
    { std::stringstream ss1( argv[3] ); int sortPrint; ss1 >> sortPrint; if(sortPrint == 1) printSorted = true; }
    ...
    int padded_size = 1;
    while(padded_size < size) padded_size *= 2;
    ...
    for(int i=0; i<size; i++) host_padded[i] = array[i];
    for(int i=size; i<padded_size; i++) host_padded[i] = INT_MAX; // Padding

    CudaSafeCall(cudaMemcpy(d_array, host_padded, padded_size * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CudaSafeCall(cudaMemcpy(host_padded, d_array, padded_size * sizeof(int), cudaMemcpyDeviceToHost));
    for(int i=0; i<size; i++) array[i] = host_padded[i];
